...
// Print attributes to screen, for debug purposses
...
float entropy(datanode data, attritibutes Attributes, string AttributeValue, int attributeindex, int targetAttribute,  int& numofExaples)
{
	...
	for (int k = 0; k < Attributes.datacell[targetAttribute].attritibutevalue.size(); k++)
	{
		...
		for (int i = 0; i <(int) data.dataline.size(); i++)
		{
			...
			{
				if (data.dataline[i].datacell[targetAttribute].attritibutevalue[0] == Attributes.datacell[targetAttribute].attritibutevalue[k])
				{
					posex++;
				}
				else
...
// ID3 algorithim 
/*
	Control all the entries in trainset is same if they are same return targetvalue. Otherwise,
	control function attribute values is empty or all of the function attributes are used then return most common value of trainset.
	else
	get best classifierindex, if index is less than 0 calculate most common value and return
	for each value in trainset calculate sub set of train set where attribute value is same with attribute value
	if the subset is empty calculate most common value in train set 
	else call id3
	@param datanode training set
	@param int target attribute
	@param attributes target attribute
	@param tree * node
	@ bool * used indexes
*/
...
void printRules(Tree* t, string rootfunction) 
{
	...
	{
		if (t->nodes.size() != 0)
			rootfunction += "Attribute["+std::to_string(t->decisionAttribute) + "] == " + t->label + " and ";
		else  
		{
...
/*
	! get evalueation attribute and decision tree and calculates gussed value 
	! controls that value with evaluation set and returns true if they are same or false otherwise
	@param ref attritibutes: evaluation attributes
	@param * tree: decision tree pointer which stores learned function based on training set
	@ret bool : result of learned function wrt. evaluation set
...
/*
	!search tree for evaluation set and return accuracy rate
	!calculate true and false predictions based on decision tree 
	@param ref datanode: data to evaluate decision tree
	@param * tree: decision tree pointer which stores learned function based on training set
	@ret float : accuracy value based on tree search
*/
